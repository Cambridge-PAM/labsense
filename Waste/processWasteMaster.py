...
DEFAULT_PATH = Path(
    r"C:\Users\takas\University of Cambridge\MET_PAM - Documents\Lab Management and Safety\Safety\Waste\Waste Master.xlsx"
)
...
def create_html_dashboard(
    ...
    html_lines = [
        ...
        f"  <title>Waste Dashboard - {display_prefix}</title>",
        ...
        "<body>",
        '<div class="container">',
        f"  <h1>Waste Dashboard — {display_prefix}</h1>",
    ...
    if plot_q and Path(plot_q).exists():
        html_lines += [
            ...
            f'  <img src="{Path(plot_q).name}" alt="Quarter stacked plot" />',
        ]
    else:
    ...
    html_lines += [
        ...
        "<table><thead><tr><th>Quarter</th><th>Volume (L)</th></tr></thead><tbody>",
    ]
    ...
    if plot_y and Path(plot_y).exists():
        html_lines += [
            ...
            f'  <img src="{Path(plot_y).name}" alt="Year stacked plot" />',
        ]
    else:
    ...
    html_lines += [
        "<h3>Year totals</h3>",
        "<table><thead><tr><th>Year</th><th>Volume (L)</th></tr></thead><tbody>",
    ]
...
def main(argv=None):
    ...
    p.add_argument(
        "--plot-dir",
        default=None,
        help="Directory to write plots to (defaults to same dir as --out)",
    )
    p.add_argument("--no-plots", action="store_true", help="Disable plot generation")
    p.add_argument(
        "--dashboard",
        action="store_true",
        help="Generate an HTML dashboard (written to plot dir)",
    )
    p.add_argument(
