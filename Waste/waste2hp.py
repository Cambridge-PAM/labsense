...
def clean_dataframe(df):
    ...
    ordered = [c for c in ['reference', 'chemical_name', 'num_containers', 'container_size', 'physical_state', 'hazard_statements', 'hazard_properties'] if c in df.columns]
    df = df[ordered]
    ...
    for i in range(1, 16):
        hp_col = f'HP{i}'
        df[hp_col] = df['hazard_properties'].apply(lambda lst, h=hp_col: int(h in lst) if isinstance(lst, list) else 0).astype(int)

    # Normalize numeric container counts if present
    if 'num_containers' in df.columns:
        df['num_containers'] = pd.to_numeric(df['num_containers'], errors='coerce').fillna(0).astype(int)
...
def main(argv=None):
    ...
    p.add_argument('--out-csv', default=None, help='Output CSV path. If not provided, defaults to hazardous_waste_table_cleaned_<date>.csv in the same folder as this script')
    p.add_argument('--out-json', default=None, help='Output JSON path. If not provided, defaults to hazardous_waste_table_cleaned_<date>.json in the same folder as this script')
    p.add_argument('--date', default=date.today().isoformat(), help='Date to append to each row (YYYY-MM-DD)')
    ...
    out_csv = Path(args.out_csv) if args.out_csv else base / f'hazardous_waste_table_cleaned_{args.date}.csv'
    out_json = Path(args.out_json) if args.out_json else base / f'hazardous_waste_table_cleaned_{args.date}.json'
