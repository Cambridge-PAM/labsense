...
def insert_sql(daily_consumption,date):
        ...
        try:
            ...
            VALUES (?,?)''',(daily_consumption,date)) #insert into table
...
def main():
    ...
    midnight_previous_day = datetime.datetime.combine(now.date() - datetime.timedelta(days=1), datetime.time.min)
    
    timestamp_seconds = int(midnight_previous_day.timestamp())
    timestamp_milliseconds_previous_day = timestamp_seconds * 1000 #finding UNIX_MILLISECOND timestamp at previous midnight


    url_daily="http://10.247.40.36/feed/data.json?id=21&start="+str(timestamp_milliseconds_previous_day)+"&end="+str(timestamp_milliseconds)+"&mode=daily&apikey=APIKEY" #details about how url is formed can be found here https://emoncms.org/site/api#feed , add your own APIKEY
